...
import pytest
from linkedin_scraper import JobScraper, JobSearchScraper, SavedJobsScraper
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="Job search selector '.jobs-search__results-list' not found - LinkedIn page structure may have changed"
)
async def test_job_search_scraper(
    browser_with_session, test_job_search_params, silent_callback
):
...
async def test_job_scraper(
    ...
    search_scraper = JobSearchScraper(
        browser_with_session.page, callback=silent_callback
    )
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_saved_jobs_scraper(browser_with_session, silent_callback):
    """Test saved jobs scraping functionality."""
    scraper = SavedJobsScraper(browser_with_session.page, callback=silent_callback)
    job_urls = await scraper.scrape(limit=5, max_scrolls=2)

    assert isinstance(job_urls, list)
    assert len(job_urls) <= 5
